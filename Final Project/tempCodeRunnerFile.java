...
public class AnimalCatalogueSystem implements AnimalOperations {
    ...
    @Override
    public void searchForAnimals(String breed, String healthStatus, int age) {
        List<Animal> results = new ArrayList<>();
        for (Animal animal : animals) {
            if ((breed == null || breed.equals(animal.getBreed())) &&
                (healthStatus == null || healthStatus.equals(animal.getHealthStatus())) &&
                (age == -1 || age == animal.getAge())) {
                results.add(animal);
            }
        }
